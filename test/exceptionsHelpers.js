...
async function tryCatch(promise, reason) {
    try {
        ...
        assert(error.message.search(errorString + reason) >= 0, "Excepted an error containing '" + errorString + reason + "' but got '" + error.message + "' instead");
    }
};
...
module.exports = {
    ...
    catchStackOverflow: async function(promise) {await tryCatch (promise, "stack underflow");},
